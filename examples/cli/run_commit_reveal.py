"""CLI command to run the commit-reveal example server node."""
...
from examples.commit_reveal.dag import COMMIT_REVEAL_CUTOFF_PERCENT
from examples.commit_reveal.server.server import Server
...
# Configure logging
configure_logging(force=True, use_color=True, use_queue=True)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run a commit-reveal example subnet node",
...
python -m examples.cli.run_commit_reveal \
...
--local_rpc
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="[Deprecated] Accepted for compatibility; commit-reveal topic validators are always enabled.",
    )
    ...
    parser.add_argument(
        ...
        help="[Deprecated] Accepted for compatibility; ignored by the commit-reveal server.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        dest="publish_log_level",
        type=int,
        default=logging.DEBUG,
        help=(
            "Log level for commit/reveal publisher. "
    ...
    parser.add_argument(
        "--dag_startup_sync_min_connected_peers",
        type=int,
        default=2,
        help="Minimum connected peers to wait for before the initial commit-reveal DAG sync.",
    )
    parser.add_argument(
        "--dag_startup_sync_timeout",
        type=float,
        default=180.0,
        help="Seconds to wait for peers during the initial commit-reveal DAG sync.",
    )
    parser.add_argument(
        "--dag_startup_sync_settle_time",
        type=float,
        default=10.0,
        help="Seconds to keep syncing after the minimum peer count is reached.",
    )
    parser.add_argument(
        "--dag_startup_sync_poll_interval",
        type=float,
        default=1.0,
        help="Seconds between connected-peer checks during startup DAG sync.",
    )
    parser.add_argument(
        "--dag_missing_sync_retry_delay",
        type=float,
        default=5.0,
        help="Seconds to wait before retrying sync for missing commit-reveal DAG parents.",
    )
    parser.add_argument(
        "--publish_interval_seconds",
        type=float,
        default=5.0,
        help="Seconds between commit/reveal publish attempts.",
    )
    parser.add_argument(
        "--cutoff_percent",
        type=float,
        default=COMMIT_REVEAL_CUTOFF_PERCENT,
        help="Epoch percent boundary between commit and reveal phases.",
    )
    parser.add_argument(
        "--skip_activate_subnet",
        action="store_true",
        help="[Testing] Run consensus without submitting activate-subnet extrinsics.",
    )
...
def main() -> None:
    ...
    logger.info("Starting commit-reveal example server node...")
    ...
    try:
        server = Server(
            ...
            skip_activate_subnet=args.skip_activate_subnet,
            ...
            publish_log_level=args.publish_log_level,
            maintain_connections_log_level=args.maintain_connections_log_level,
            dag_startup_sync_min_connected_peers=args.dag_startup_sync_min_connected_peers,
            dag_startup_sync_timeout=args.dag_startup_sync_timeout,
            dag_startup_sync_settle_time=args.dag_startup_sync_settle_time,
            dag_startup_sync_poll_interval=args.dag_startup_sync_poll_interval,
            dag_missing_sync_retry_delay=args.dag_missing_sync_retry_delay,
