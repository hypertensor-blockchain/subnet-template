...
#         if not schema_id:
#             raise ValueError("PeerStateDagSchema schema_id must be a non-empty string")
...
#             raise PayloadValidationError(f"peer state payload requires kind={self.schema_id!r}")
...
#                     f"peer state nodes can only reference parents with schema_id={self.schema_id!r}"
#                 )
...
#     configured schema frontier known locally so the example demonstrates one shared,
...
#     ):
#         if not schema_id:
#             raise ValueError("PeerStateDagPublisher schema_id must be a non-empty string")
#         if not namespace:
#             raise ValueError("PeerStateDagPublisher namespace must be a non-empty string")
...
#             raise ValueError(f"Peer-state publisher requires namespace {namespace!r} to be configured with a signer")
#         try:
#             context.runtime.schema_registry.require(self.schema_id)
#         except SchemaNotFoundError as exc:
#             raise ValueError(
#                 f"Peer-state publisher requires namespace {namespace!r} to register schema_id={self.schema_id!r}"
#             ) from exc

#         self.dag = context.dag
#         self.db = dag_system.db
#         self.local_peer_id = dag_system.local_peer_id.to_string()
#         self.dag_topic = dag_topic or context.topic
...
#             )
#             if result is None:
#                 raise RuntimeError("Peer-state DAG publish unexpectedly returned no result")
...
#         snapshot = self.db.get_nested(self.dag_topic, self.local_peer_id)
...
#         previous = self.db.get_nested(self.dag_topic, self.local_peer_id)
...
#         self.db.set_nested(self.dag_topic, self.local_peer_id, current)
