...
class PeerStatusGossipReceiver:
    ...
    def __init__(
        ...
        *,
        dag_topic: str = PEER_STATE_TOPIC,
    ...
    async def _handle_message(self, message: rpc_pb2.Message) -> None:
        ...
        try:
            ...
            logger.info("Processed gossiped DAG node %s from %s with ingest result %s",
                decoded.node.header.node_id, from_peer, ingest_result.status.value)
        ...
        self._store_peer_state_snapshot(from_peer, decoded)
    ...
    def _store_peer_state_snapshot(self, peer_id: str, message: DagNodeGossip) -> None:
        """Store the latest valid peer-state snapshot in the plain DB for fast lookup."""
        if self.db is None:
            return
        ...
        previous = self.db.get_nested(PEER_STATE_TOPIC, peer_id)
        ...
        self.db.set_nested(PEER_STATE_TOPIC, peer_id, current)
